...
# Налаштування для українських графіків
...
# Очистимо пробіли в назвах колонок
df.columns = df.columns.str.strip()

# Тепер правильні назви
state_columns = [col for col in df.columns if col != 'date']

# Обчислюємо середнє по всіх штатах
...
axes[0].set_title('Динаміка середніх цін на бензин в Індії (2017-2022)',
...
p = np.poly1d(z)
axes[0].plot(df['date'], p(range(len(df))), "--", color='red',
             linewidth=2, label=f'Тренд: {z[0]:.2f}x + {z[1]:.2f}')
axes[0].legend()
