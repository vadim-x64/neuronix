...
def plot_classification_results(training_data, training_labels, test_data, test_labels,
    ...
    plt.scatter(training_data[training_labels == 0][:, 0], training_data[training_labels == 0][:, 1],
                c='blue', marker='o', s=100, alpha=0.7, label='Навчальні: Звичайні')
    plt.scatter(training_data[training_labels == 1][:, 0], training_data[training_labels == 1][:, 1],
    ...
    if len(correct_preds) > 0:
        plt.scatter(correct_preds[correct_labels == 0][:, 0], correct_preds[correct_labels == 0][:, 1],
                    c='cyan', marker='^', s=150, label='Тест: Звичайні (правильно)')
        plt.scatter(correct_preds[correct_labels == 1][:, 0], correct_preds[correct_labels == 1][:, 1],
    ...
    plt.title('Класифікація автомобілів: Спортивні vs Звичайні')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    plt.savefig('plots/graph.png', dpi=300, bbox_inches='tight')

    print("Графік збережено в файл: plots/graph.png")
